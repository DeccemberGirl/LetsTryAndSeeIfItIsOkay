namespace Lab123.Lab2Task
{
    public static class ExceptionMessagesLab2
    {
        public const string InvalidPathToFileMessage = "\nYou specified the wrong path to your file with the input data or the file does not exist.";
        public const string NotNumberInputMessage = "\nInput value in the file was not a number.";
        public const string ExceedingLimitsNumberMessage = "\nInput value in the file exceeds the limits: only numbers from 2 to 100 are allowed.";
        public const string DifferentNumberOfStairsMessage = "\nNumber of input values in the second line of the file is different from the number in the first line given.";
    }
}
